...
| __Query frequency__   | hundreds per day                   |
| __SQL__ |```SQL  SELECT "Reviews".rating, "Reviews".comment, "Reviews".review_date, "Users".username, "Users".img, "Users".user_id
FROM (("Reviews" INNER JOIN "SerialKeys" ON "SerialKeys".sk_id = "Reviews".sk_id) INNER JOIN "Users" ON "SerialKeys".owner_id = "Users".user_id) WHERE "SerialKeys".product_id = $productId ORDER BY "Reviews".rating DESC;  ```          |
