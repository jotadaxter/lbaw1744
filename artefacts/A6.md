...
| Clustering      | No              |
| Cardinality | High |
| Justification   | Query SELECT01 has to be fast as it is executed many times; doesn't need range query support; cardinality is high because username of the user is an unique key; it's not a good candidate for clustering.   |
