...
### 1.2. Frequent Queries
 ```SQL  SELECT *  FROM "Products"  WHERE "Products".product_id = $productId; ```
...
| __Query frequency__   | hundreds per day                   |
| __SQL__ |```SQL SELECT product_id FROM "Discounts WHERE (CURRENT_DATE BETWEEN start_date AND end_date)" ```          | 
