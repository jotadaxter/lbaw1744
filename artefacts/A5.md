...
| R05                | SerialKey(__sk_id__, code UK NN, purchase_id → Purchase, user_id → User, product_id → Product, cost)                       |
...
| **Functional Dependencies** ||
| FD0501                  | {sk_i} -> {code, purchase_id, user_id, product_id, cost} |
| FD0502                  | {code} -> {sk_id, purchase_id, user_id, product_id, cost} |
| FD0503                  | {product_id} -> {product_id, sk_id, code, purchase_id, user_id} |
| **Normal Form:** | BCNF       |
