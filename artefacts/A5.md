...
| R05                | SerialKey(__activationCode_id__, code UK NN, purchase_id → Purchase, user_id → User)                       |
| R06              | Purchase(__purchase_id__ , paid_date NN, price NN, details, method NN CK method IN PaymentMethod, user_id → user) |
| R07                | Review(__review_id__ , date NN, rating NN CK rating > 0 AND rating <= 5, comment NN)            |
...
| R15                | Purchase_Product(__purchase_id__ → Purchase, __product_id__ → Product)    |
