...
| R04                | Product(__product_id__, name NN, description NN, release_date NN, system NN, price NN CK price >= 0, logo)      |
| R05                | Discount(__discount_id__, discounted_price NN CK discounted_price > 0 AND discounted_price < 1, begin_date NN CK begin_date > end_date, end_date NN, product_id → Product)                       |
| R06                | SerialKey(__activationCode_id__, code UK NN)                       |
| R08                | Purchase(__purchase_id__ , paid_date NN, price NN, details, method NN CK method IN PaymentMethod)    |
...
| **Keys:** | { payment_id }    |
| **Functional Dependencies** ||
| FD1001                  |{payment_id} -> {paid_date, method, total, details}|
| **Normal Form:** | BCNF       |
...
| **Keys:** | { review_id }    |
| **Functional Dependencies** ||
| FD1401                  | {review_id} -> {purchase_id, user_id}|
| **Normal Form:** | BCNF       |
